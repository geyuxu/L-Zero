...
L-0 is a **custom virtual machine** with its own instruction set (ISA). Programs are written in **L-0 Assembly** (`.asm`), compiled to **bytecode** (`.l0`), and executed by `l0vm`. Optionally, programs can be **AOT compiled** to native C code via `l0cc`.

**CRITICAL FOR AI AGENTS:**
- **Output Format: ASM ONLY** - Do NOT generate JSON instruction arrays
- Use **Labels** for control flow (e.g., `JMP Loop`) - never calculate line numbers
- Use **inline comments** (`# comment`) to explain your reasoning
- The `.asm` file is your source; `.l0` bytecode is the executable

```bash
# Standard workflow (ASM → Binary → Execute)
./l0asm program.asm > program.l0   # Compile ASM to bytecode
./l0vm program.l0                   # Execute bytecode

# AOT compilation (for maximum performance)
./l0cc program.l0 -o program.c      # Generate C code
gcc -O2 program.c -o program        # Compile to native
...
| Aspect | ASM | JSON |
|--------|-----|------|
| Token efficiency | High (minimal syntax) | Low (lots of `{}[]":,`) |
| Control flow | Labels (`JMP Loop`) | Line numbers (`JMP 15`) - error-prone |
| Comments | Inline (`# reasoning`) | Not supported |
| Parsing | Robust (one line = one instruction) | Fragile (missing `]` breaks everything) |
...
| Format | Purpose | AI Output? |
|--------|---------|------------|
| `.asm` | **Source code** (labels, comments) | **YES** - AI generates this |
| `.l0`  | Binary bytecode (execution) | No - compiled artifact |
| `.c`   | AOT compiled (native performance) | No - generated by l0cc |
| `.json`| ~~Deprecated~~ (use --dev flag) | NO - avoid |
